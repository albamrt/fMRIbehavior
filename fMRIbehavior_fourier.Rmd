...
# Relevel factors:
data$group <- relevel(data$group, ref = 'E')
...
```{r fig.show='hold', fig.align='center', fig.cap = 'Coloured lines accounts for the linear model fitted for the uniform component (subject by subject). Black lines are the results of the linear model fitted on the intercept and slope of the subject-models (coloured lines).', warning = FALSE}
uniform_mod <- groupedSubjSes %>%
  ungroup() %>%
  group_by(subject) %>%
  nest() %>%
  mutate(mod = map(data, function(df) glm(Pu ~ time , data = df,
                                          family = binomial(link = "logit"), weights=n)),
         coefficients = map(mod, function(df) df$coefficients),
         intercept = map_dbl(coefficients, 1),
         slope = map_dbl(coefficients, 2), 
         group = substr(subject, 1, 1),
         prediction = map(mod, function(x) predict(x, type = 'response')),
         time = map(data, function(x) x$time))

pred <- expand.grid(group = c('C', 'E', 'S'), time = unique(groupedSubjSes$time))
pred$intercept <- predict(lm(intercept ~ group, data = uniform_mod), 
                          newdata = pred)
pred$slope <- predict(lm(slope ~ group, data = uniform_mod), 
                      newdata = pred)
pred <- pred %>%
  mutate(prediction = exp(intercept +slope*time))
...
#load(file = '/Users/albamorato/Documents/IDIBAPS/ANMDA/MRI/behaviour/simm.RData')
#load(file = '/Users/albamorato/Documents/IDIBAPS/ANMDA/MRI/behaviour/simm3.RData')
...
groupedSubjSes <- groupedSubjSes %>%
  mutate(foldedserialbias = map(.x = data, .f = function(x) 
    folded_serial_bias(prevcurr = x$diffstim, error = x$fourier_error, window = window,
                       step = step)),
    mean_folded = unlist(map(.x = foldedserialbias, function(x) mean(x$m))))
