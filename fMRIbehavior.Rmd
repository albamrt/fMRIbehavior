...
knit: (function(input_file, encoding) {
  ...
  encoding = encoding,
  output_file = file.path(dirname(input_file), out_dir, 'fMRIbehavior.html'))})
...
```{r, fig.align='center', message = FALSE, warning = FALSE, fig.height = 40, fig.width = 8, fig.cap = 'Mean error by subject trough time'}
data %>%
  ggplot(aes(x = time, y = abs(error), color = group)) +
  geom_jitter(width = 0.05, alpha = 0.2, show.legend = FALSE) +
  stat_summary(fun.y = mean, geom = 'point', size = 1, show.legend = FALSE) +
